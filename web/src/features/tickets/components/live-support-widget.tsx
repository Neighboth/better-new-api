...
import { createTicket, getTicketConfig, getUserTickets } from '../api'
...
export function LiveSupportWidget() {
  ...
  const handleSendQuickMessage = async (e: React.FormEvent) => {
    ...
    if (!auth.user) {
      toast.error(t('Please log in to contact support'))
  ...
  return (
    <div className='fixed bottom-5 right-5 z-50'>
      {open ? (
        <Card className='w-[340px] sm:w-[380px] shadow-2xl border-border animate-in fade-in zoom-in-95 duration-150 overflow-hidden flex flex-col'>
          {/* Widget Header */}
          ...
          {/* Widget Body */}
          <CardContent className='p-4 space-y-4 max-h-[360px] overflow-y-auto text-sm'>
            {!auth.user ? (
              <div className='text-center py-6 space-y-3'>
                <p className='text-muted-foreground text-xs'>
                  {t('Please log in to chat with support or submit a ticket.')}
                </p>
                <Button
                  size='sm'
                  onClick={() => {
